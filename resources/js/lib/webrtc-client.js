...
export class WebRTCClient extends EventTarget {
	constructor(options) {
		...
		this.userName = options.userName; // Set userName
		...
		this._state = {
			...
			screenEnabled: false,
			localStream: null,
			peers: new Map(), // peerId -> { peerId, userId, userName, audioEnabled, videoEnabled, screenEnabled, speaking }
		...
		case "peer_joined":
			{
				const peer = {
					peerId: msg.peerId,
					userName: msg.userName, // Map userName
					userId: msg.userId,
